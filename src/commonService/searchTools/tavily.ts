...
import { API_TIMEOUTS, MAX_RESULTS } from '../utils/constants.ts';
import { SearchResult, ImageResult } from '../utils/types.ts';
...
export async function tavilySearch(query: string): Promise<{ web: SearchResult[]; images: ImageResult[] }> {
  try {
    ...
    // Type assertion to help TypeScript understand the response type
    const data = response.data as TavilyResponse;
    ...
    const web = Array.isArray(data.results) ? data.results
      .filter((result: TavilyResult) => isValidUrl(result.url))
      .filter((result: TavilyResult) => result.score > 0.6) // Only keep high relevance results
    ...
    const images = Array.isArray(data.images) ? data.images
      .filter((img: { url: string; title?: string; description?: string }) => isValidUrl(img.url))
      .filter((img: { url: string; title?: string; description?: string }) => {
        try {
      ...
      .map((img: { url: string; title?: string; description?: string }) => ({
        url: img.url,
